...
#data parameters
vect_n = 16 #size of vectors of data
data_n = int(0.6*(10**5))
#robot parametrs
...
def reg_accuracy(y_true, y_pred):
    return 100*K.mean(  K.less_equal( K.abs(y_pred - y_true), 0.013 )  )
# Network building
...
model.add(Dense(4))
model.compile(loss='mean_squared_error', optimizer=keras.optimizers.Adadelta() ,metrics=[reg_accuracy])

#training
no_epochs = 150
history = model.fit(pose_data, joints_data, epochs=no_epochs, batch_size=200, validation_split=0.15)
...
print("\n loss: %f, accuracy: %f" % (scores[0], scores[1])) 
...
plt.plot(history.history['reg_accuracy'])
plt.plot(history.history['val_reg_accuracy'])
