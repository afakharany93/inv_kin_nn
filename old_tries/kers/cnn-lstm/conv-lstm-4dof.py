...
# Network buildingless_equal(
# create an) d fit the LSTM network
model = Sequential()
...
model.add(Dense(4))
model.compile(loss='mean_squared_error', optimizer='adam' ,metrics=[reg_accuracy])
...
for i in range(report_interval):
    print('intreval = ', i+1)
    model.fit(pose_data, joints_data, epochs=int(no_epochs/report_interval), batch_size=500, validation_split=0.15)
